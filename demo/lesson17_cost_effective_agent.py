...
print("=================================================================")
print(f"🔒 [安全检查] 当前加载的 Key: {mask_key(DEEPSEEK_API_KEY)}----{mask_key(TAVILY_API_KEY)}")
...
# 初始化 DuckDuckGo 搜索
# ddg_search = DuckDuckGoSearchResults(region="cn",output_format="json", max_results=2)
ddg_search = TavilySearch(
    ...
    try:
        ...
        for item in items[:2]:
            simplified.append({
                ...
                "snippet": (item.get("content") or item.get("snippet", ""))[:120],
...
def run_agent_task(user_query: str):
    ...
    # 安全锁 1 & 2： 限定 max_tokens 与 temperature，杜绝废话发送
